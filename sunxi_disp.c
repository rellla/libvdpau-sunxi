...
static int sunxi_disp_set_osd_layer(struct sunxi_disp *sunxi_disp, int x, int y, int width, int height, output_surface_ctx_t *surface)
{
	...
	disp->osd_info.src_win.x = surface->rgba_p->dirty.x0;
	disp->osd_info.src_win.y = surface->rgba_p->dirty.y0;
	disp->osd_info.src_win.width = surface->rgba_p->dirty.x1 - surface->rgba_p->dirty.x0;
	disp->osd_info.src_win.height = surface->rgba_p->dirty.y1 - surface->rgba_p->dirty.y0;
	disp->osd_info.scn_win.x = x + surface->rgba_p->dirty.x0;
	disp->osd_info.scn_win.y = y + surface->rgba_p->dirty.y0;
	disp->osd_info.scn_win.width = min_nz(width, surface->rgba_p->dirty.x1) - surface->rgba_p->dirty.x0;
	disp->osd_info.scn_win.height = min_nz(height, surface->rgba_p->dirty.y1) - surface->rgba_p->dirty.y0;
